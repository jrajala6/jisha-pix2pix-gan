...
class Trainer:
    ...
    def __init__(self, args):
        ...
        # Auto-resume: check for latest.pth before building data
        ...
        # If resuming, peek-load attribute columns to enforce same column order
        self._forced_attribute_columns = None
        if args.resume_from:
            ckpt_peek = torch.load(args.resume_from, map_location='cpu', weights_only=False)
            self._forced_attribute_columns = ckpt_peek.get('dataset_meta', {}).get('attribute_columns', None)
            if self._forced_attribute_columns:
                print(f"  Will enforce {len(self._forced_attribute_columns)} attribute columns from checkpoint")
            del ckpt_peek

        # Setup data (uses forced columns if resuming)
        self._setup_data()
    ...
    def _setup_data(self):
        ...
        # Create data loaders (pass forced columns if resuming for exact reproducibility)
        kwargs = dict(
        ...
        if self._forced_attribute_columns:
            kwargs['attribute_columns'] = self._forced_attribute_columns

        self.train_loader, self.val_loader = create_dataloader(**kwargs)
    ...
    def train(self):
        ...
        for epoch in range(self.current_epoch, self.args.epochs):
            ...
            # Save checkpoint
            is_best = False
            if val_stats and val_stats['val_l1_loss'] < self.best_loss:
                self.best_loss = val_stats['val_l1_loss']
                is_best = True
